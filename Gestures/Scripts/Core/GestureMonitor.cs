...
namespace Gestures
{
    ...
    public class GestureMonitor : MonoBehaviour
    {
        ...
        void SmoothTransform(List<GTransform> transforms) {
            StartCoroutine(N(transforms));
        }

        IEnumerator N(List<GTransform> data) {
            while (true) {
                List<GTransform> temp = viewNormalizer.Normalize(data);
                RenderList(temp);
